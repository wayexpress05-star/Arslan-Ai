...
// Simple error message template
const messageTemplates = {
    error: (message) => ({
        text: message
    }),
    success: (text, imageUrl, type) => ({
        image: { url: imageUrl },
        caption: `✨ Made by Arslan-MD ✨\n\n🎨 Effect: ${type.toUpperCase()}\n📝 Text: ${text}`
    })
};
...
async function textmakerCommand(sock, chatId, message, q, type) {
    try {
        ...
        try {
            ...
            await sock.sendMessage(chatId, messageTemplates.success(text, result.image, type));
        } catch (error) {
